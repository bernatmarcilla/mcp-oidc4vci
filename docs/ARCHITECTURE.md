...
**Signed metadata.** The spec allows the well-known endpoint to return either an unsigned JSON document or a signed JWT containing the same claims (spec "Signed Metadata", §12.2.3) — a Credential Issuer chooses which per request, generally by honoring the Wallet's `Accept` header. The default fetcher sends `Accept: application/json, application/jwt` (accepting either), and `_parse_metadata_payload` sniffs the response body's shape to decide which parser to use, rather than trusting the `Content-Type` header — real issuers have already proven inconsistent about that (Phase 8 in [docs/ROADMAP.md](ROADMAP.md), internal-only). When it's a JWT, the JOSE header's `typ` must be exactly `openidvci-issuer-metadata+jwt` and its `alg` must not be `none` or a symmetric (MAC) algorithm, both rejected explicitly rather than left to whatever the underlying JWT library happens to allow. The signature is verified against the public key in the header's `x5c` certificate chain (its leaf certificate) — the only one of the spec's three key-conveyance mechanisms (`x5c`, `kid`, `trust_chain`) this implementation supports; a token that only carries `kid` or `trust_chain` is rejected, not silently accepted unverified. The payload's `sub` claim must match the requested Credential Issuer identifier, `iat` must be present, and `exp`, if present, must not have passed.

**Known scope limit — no certificate chain-of-trust validation.** Verifying the JWS signature only proves the response wasn't altered after signing by whoever holds the `x5c` leaf certificate's private key; it does not establish that that key belongs to a trustworthy party, since this implementation does not build or validate the certificate chain against any trust anchor. The spec is explicit that concrete trust-establishment mechanisms are out of scope for it too — it just requires a Wallet to have *some* answer, or reject the metadata outright. Full chain-of-trust validation (e.g. against the public CA trust store, the same root of trust the HTTPS fetch itself already relies on) was considered and deliberately deferred: the spec's own non-normative example (Appendix I.1) signs with a self-signed certificate, which a public-CA validation model would reject outright, and this project has no real-world signed-metadata issuer to validate a chosen trust model against yet. Revisit if that becomes available.
...
**Credential Issuer Metadata as signed JWT:** the same environment's `.well-known/openid-credential-issuer` endpoint defaults to `Content-Type: application/jwt` — the entire response is a signed JWT (`typ: openidvci-issuer-metadata+jwt`, `x5c` certificate chain) rather than plain JSON, unless a client explicitly signals it accepts JSON. `get_credential_issuer_metadata` now handles either representation and verifies the signature when a JWT comes back — see [Architecture](ARCHITECTURE.md#get_credential_issuer_metadata) for the parsing and trust-model details.
...
**Multiple credential configurations per offer.** The spec's Credential Request only ever names one `credential_configuration_id` — it has no batch form for requesting several *different* credential types in a single call (`proofs.jwt` batches multiple proofs of the *same* type, which is a separate, still-unimplemented feature — see the Feature support table in the [README](../README.md)). So an offer requesting several configurations needs one Credential Request per configuration. The session tracks progress via `next_credential_index`: `request_credential`, `submit_wallet_proof`, and `poll_deferred_credential` each handle exactly the configuration at that index and, if more remain, return the session to `ready_for_credential_request` (not `completed`) rather than looping internally — the caller calls the same tool again for the next one. This sidesteps having to track an automatic-vs-manual continuation mode: every entry point applies the same one-configuration-per-call rule uniformly. Also always includes a `jwt` proof unconditionally — the spec only requires `proofs` when the configuration declares `proof_types_supported`, but sending one unconditionally is harmless and keeps the logic simple.
